...
import { FacturaPorPagar, Proveedor, ItemFactura, TipoDocumento } from '../../types/cuentasPorPagar';
import { useSesion } from '../../context/SesionContext';
import { TipoDocumentoFactura } from '../../types/nomencladores';
import { SearchableProveedorSelector } from './SearchableProveedorSelector';
import { NotificationModal } from '../../components/common/NotificationModal';
import { useModals } from '../../hooks/useModals';

interface FacturaPagarModalProps {
  isOpen: boolean;
  onClose: () => void;
  factura?: FacturaPorPagar | null;
  proveedores: Proveedor[];
  mode: 'create' | 'edit' | 'view';
  onSave: (factura: Omit<FacturaPorPagar, 'id' | 'fechaCreacion' | 'fechaModificacion'>) => Promise<string>;
  tiposDocumento: TipoDocumentoFactura[];
}

export const FacturaPagarModal: React.FC<FacturaPagarModalProps> = ({
  isOpen,
  onClose,
  factura,
  proveedores,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Buscar proveedor con validación mejorada
      const proveedor = proveedores.find(p => p.id === formData.proveedorId);
      
      if (!proveedor) {
        console.error('Proveedor no encontrado:', {
          proveedorId: formData.proveedorId,
          proveedoresDisponibles: proveedores.map(p => ({ id: p.id, nombre: p.nombre }))
        });
        
        // Mostrar error con NotificationModal en lugar de alert
        showError(
          'Proveedor no encontrado',
          `No se pudo encontrar el proveedor con ID: ${formData.proveedorId}. Por favor, seleccione un proveedor válido.`
        );
        
        return;
      }

      const facturaData: Omit<FacturaPorPagar, 'id' | 'fechaCreacion' | 'fechaModificacion'> = {
        ...formData,
        proveedor,
        ...
        <div className="sticky top-0 bg-white z-10 p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-900">
              {mode === 'create' ? 'Registrar Factura de Proveedor' : 
               mode === 'edit' ? 'Editar Factura de Proveedor' : 'Ver Factura de Proveedor'}
            </h2>
            <button
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <SearchableProveedorSelector
                proveedores={proveedores}
                value={formData.proveedorId}
                onChange={(proveedorId) => setFormData({ ...formData, proveedorId })}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {mode !== 'view' && (
                <button
                  ...
                  onClick={addItem}
                  disabled={saving}
                  className="bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 disabled:opacity-50 flex items-center gap-1 text-sm"
                >
                    ...
                    <tr key={item.id}>
                      ...
                      <td className="px-4 py-2">
                        <input
                          ...
                          className="w-20 px-2 py-1 border border-gray-300 rounded text-sm text-center focus:ring-1 focus:ring-red-500"
                          min="0"
                          step="0.01"
                          disabled={mode === 'view' || saving}
                        />
          ...
          <div className="sticky bottom-0 bg-white pt-4 pb-4 border-t border-gray-200 flex justify-end space-x-3">
            ...
            {mode !== 'view' && (
              <button
                type="submit"
                disabled={saving || !formData.proveedorId || items.every(item => !item.descripcion.trim())}
                className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 min-w-[140px]"
              >
                {saving ? (
                  <>
                  ...
                  <>
                    ...
                    <span>{mode === 'create' ? 'Registrar Factura' : 'Actualizar Factura'}</span>
                  </>
                )}
              </button>
            )}
