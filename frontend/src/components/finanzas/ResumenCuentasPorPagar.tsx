...
import { ResumenCuentasPorPagar as ResumenCuentasPorPagarType } from '../../types/cuentasPorPagar';

interface ResumenCuentasPorPagarProps {
  resumen: ResumenCuentasPorPagarType | null;
  loading?: boolean;
}

export const ResumenCuentasPorPagar: React.FC<ResumenCuentasPorPagarProps> = ({ 
  resumen, 
  loading = false 
}) => {
  ...
  const cards = [
    ...
    {
      title: 'Promedio Pago',
      value: `${resumen.promedioPago} días`,
    ...
    },
    {
      ...
      value: resumen.proveedoresConDeuda.toString(),
  ...
  return (
    <div className="space-y-6">
      ...
      {resumen.proveedoresMayorDeuda.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Proveedores con Mayor Deuda</h3>
          <div className="space-y-3">
            {resumen.proveedoresMayorDeuda.slice(0, 5).map((proveedor, index) => (
              <div key={proveedor.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{proveedor.numeroDocumento}</p>
                  </div>
                </div>
