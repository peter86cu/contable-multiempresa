...
function validateEnvironmentVariables() {
  ...
  const missingVars = Object.entries(requiredVars).filter(([_, value])=>!value).map(([key, _])=>key);
...
async function getAuth0ManagementToken(domain, clientId, clientSecret) {
  try {
...
async function getAuth0Users(domain, token, params) {
  try {
...
async function createAuth0User(domain, token, userData) {
  try {
...
async function updateAuth0User(domain, token, userId, userData) {
  try {
...
async function deleteAuth0User(domain, token, userId) {
  try {
...
function getMockResponse(method, pathSegments, effectivePathLength, body) {
  const mockUsers = getMockUsers();
  if (method === 'GET' && effectivePathLength === 0) {
    // Devolver lista de usuarios mock
    const usuariosMapeados = mockUsers.map((u)=>({
        id: u.user_id,
        email: u.email,
    ...
    return new Response(JSON.stringify(usuariosMapeados), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json',
        'X-Mock-Data': 'true'
    ...
    const mockUser = mockUsers.find((u)=>u.user_id === decodedUserId || u.user_id === `auth0|${decodedUserId}` || decodedUserId.includes(u.user_id.replace('auth0|', ''))) || mockUsers[0];
    ...
    return new Response(JSON.stringify(usuarioMapeado), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json',
        'X-Mock-Data': 'true'
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    let managementToken;
    try {
      managementToken = await getAuth0ManagementToken(envVars.AUTH0_DOMAIN, envVars.AUTH0_MGMT_CLIENT_ID, envVars.AUTH0_MGMT_CLIENT_SECRET);
    } catch (tokenError) {
